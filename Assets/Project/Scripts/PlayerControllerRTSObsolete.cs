...
namespace Project
{
	...
	public class PlayerControllerRTSObsolete : MonoBehaviour
	{
		...
		private InteractableObjectObsolete2 _currentInteractable;
		...
		private void SetupFSM()
		{
			...
			_fsm.AddState("GoToInteractionPoint",
				onEnter =>
				{
					...
					_stateElapsed = 0;
					_agent.stoppingDistance = _currentInteractable.InteractionDistance;
			...
			_fsm.AddState("StartInteraction",
				onEnter =>
				{
					...
					_currentInteractable.Interact(InteractionStateType.Start);
				},
				onLogic =>
				{
					_stateElapsed -= Time.deltaTime;
			...
			_fsm.AddState("Interacting",
				onEnter =>
				{
					...
					if (animationData) animationData.Apply(_animator);
					_currentInteractable.Interact(InteractionStateType.Loop);
					_agent.enabled = true;
					_agent.ResetPath();
				},
				onLogic =>
			...
			_fsm.AddState("EndInteraction",
				onEnter =>
				{
					...
					if (animationData) animationData.Apply(_animator);
					_currentInteractable.Interact(InteractionStateType.End);
				},
				onLogic =>
				{
					_stateElapsed += Time.deltaTime;
		...
		private IEnumerator DelayedMoveRTS(Vector2 pointerPosition)
		{
			...
			if (Physics.Raycast(ray, out RaycastHit hitInfo, 1000, _layerMask))
			{
				...
				_shouldMove = true;
				if (hitInfo.transform.TryGetComponent(out InteractableObjectObsolete2 interactable))
				{
